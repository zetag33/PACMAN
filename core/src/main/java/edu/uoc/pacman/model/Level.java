...
public class Level {
    ...
    public boolean isPathable(Position position) {
        MapItem m = getMapItem(position);
        return m != null && m.isPathable();
    }
    ...
    public void setGhostsFrightened() {
        for (Ghost g : ghostList) {
            g.setBehaviour(Behaviour.FRIGHTENED);
        }
    }
    ...
    public boolean hasWon() {
        for (MapItem m: mapItemList) {
            if (m instanceof Scorable && m instanceof Pickable p && !p.isPicked()) {
                return false;
            }
        }
        return true;
